...
class OutcomeSamplingSolver(mccfr.MCCFRSolverBase):
  ...
  def __init__(self, game, regressor_update_freq=np.inf, max_infostates=np.inf, reset_type=mccfr.ResetType.NONE):
    ...
    # current_policy.
    self._expl = 0.6
    self._regressor_update_freq = regressor_update_freq
  ...
  def iteration(self):
    ...
    if self._internal_iteration % self._regressor_update_freq == 0:
      self._update_regressor()
  ...
  def _episode(self, state, update_player, my_reach, opp_reach, sample_reach):
    ...
    if cur_player == update_player:
      ...
      for aidx in range(num_legal_actions):
        ...
        self._add_regret(info_state_key, legal_actions[aidx], cf_action_value - cf_value)

      # update the average policy
      for aidx in range(num_legal_actions):
        increment = my_reach * policy[aidx] / sample_reach
        self._add_avstrat(info_state_key, legal_actions[aidx], increment)
