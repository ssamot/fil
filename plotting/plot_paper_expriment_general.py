...
def load_and_plot(data, random_exploitability, file_identifier, use_log_scale):
    for d in data:
        check_data(d["data"])

        data_mean = np.mean(d["data"]["y"], axis=0)
        n = len(d["data"]["y"])
        sem = stats.sem(d["data"]["y"], axis=0)
        confidence = conf.CONFIDENCE
        ci = sem * stats.t.ppf((1 + confidence) / 2., df=n-1)
        d["mean"] = data_mean
        d["ci"] = ci

    plt.figure(figsize=(conf.FIGX,conf.FIGY))
    ...
    plt.tight_layout()
    if use_log_scale:
        plt.savefig(f"results/fig_{file_identifier}_log.pdf", dpi=300)
    else:
        plt.savefig(f"results/fig_{file_identifier}.pdf", dpi=300)
