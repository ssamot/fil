...
"""Implements Deep CFR Algorithm with Feature Interaction Layers.
...
class MLP(nn.Module):
  ...
  def __init__(self,
    ...
    self.encoder = fil_torch_ohi.FeatureInteractionLayer(cat_split_dims, cat_real_dims, groups)
    dummy = torch.zeros((1, input_size), dtype=torch.long)
    current_index = 0
    for real_dim, split_dim in zip(cat_real_dims, cat_split_dims):
      if real_dim == split_dim:
        dummy[0][current_index] = 1
      current_index += split_dim
...
class DeepCFRSolver(policy.Policy):
  ...
  def __init__(self,
    ...
    self._iteration = 1
    self._cat_split_dims = cat_split_dims
    self._cat_real_dims = cat_real_dims
    ...
    self._policy_network = MLP(self._embedding_size,
                               ...
                               self._num_actions, self._cat_split_dims, self._cat_real_dims, self._fil_groups)
    ...
    self._advantage_networks = [
        MLP(self._embedding_size, list(advantage_network_layers),
            self._num_actions, self._cat_split_dims, self._cat_real_dims, self._fil_groups) for _ in range(self._num_players)
    ]
