...
class DeepCFRSolver(policy.Policy):
  ...
  def __init__(self,
    ...
    infosets = {str(state.information_state_tensor()) for state in states.values()}
    self._infoset_map = {infoset: np.eye(len(infosets))[index] for index, infoset in enumerate(infosets)}
    

    # Define strategy network, loss & memory.
    self._strategy_memories = ReservoirBuffer(memory_capacity)
    self._policy_network = MLP(len(self._infoset_map),
  ...
  def action_probabilities(self, state, player_id=None):
    ...
    with torch.no_grad():
      logits = self._policy_network(torch.FloatTensor([self._infoset_map[str(state.information_state_tensor())]]))
  ...
  def _learn_strategy_network(self):
    ...
    for _ in range(self._policy_network_train_steps):
      ...
      iterations = []
      for s in samples:
        infoset_index = self._infoset_map[str(s.info_state)]
        info_states.append(infoset_index)
