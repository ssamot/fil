...
class Tabular:
  ...
  def fit(self, X, y):
    for sample_x, sample_y in zip(X, y):
      self.table[str(sample_x)] = sample_y

  def predict(self, X):
    y = []
    for sample_x in X:
      x_key = str(sample_x)
      if x_key in self.table:
        y.append(self.table[x_key])
      else:
        y.append(np.ones(self.action_size))
    return y
...
class DeepCFRSolver(policy.Policy):
  ...
  def action_probabilities(self, state, player_id=None):
    ...
    probs = self._policy_regressor.predict([info_state])[0]
    probs[legal_actions] /= probs[legal_actions].sum()
  ...
  def _learn_advantage_network(self, player):
    ...
    y = []
    for s in samples:
      info_state_key = str(s.info_state)
      if info_state_key not in num_samples_per_infoset:
        info_state_index[info_state_key] = len(num_samples_per_infoset)
        num_samples_per_infoset[info_state_key] = s.iteration
        X.append(s.info_state)
        y.append(np.array(s.advantage) * s.iteration)
      else:
        num_samples_per_infoset[info_state_key] += s.iteration
        y[info_state_index[info_state_key]] += np.array(s.advantage) * s.iteration
    for info_state_key in num_samples_per_infoset:
      y[info_state_index[info_state_key]] /= num_samples_per_infoset[info_state_key]

    self._advantage_regressors[player] = create_regressor(self._num_actions)
    self._advantage_regressors[player].fit(X, y)
  ...
  def _learn_strategy_network(self):
    ...
    for s in samples:
      ...
      if info_state_key not in num_samples_per_infoset:
        ...
        num_samples_per_infoset[info_state_key] = s.iteration
        X.append(s.info_state)
        y.append(np.array(s.strategy_action_probs) * s.iteration)
      else:
        num_samples_per_infoset[info_state_key] += s.iteration
        y[info_state_index[info_state_key]] += np.array(s.strategy_action_probs) * s.iteration
    for info_state_key in num_samples_per_infoset:
      y[info_state_index[info_state_key]] /= num_samples_per_infoset[info_state_key]

    self._policy_regressor = create_regressor(self._num_actions)
    self._policy_regressor.fit(X, y)