...
class DeepCFRSolver(policy.Policy):
  ...
  def wrapped_travesral_with_player(self, p):
    return self._traverse_game_tree(self._root_node, p)
  ...
  def solve(self):
    ...
    Returns:
      ...
      3. (float) Policy loss.
    """
    player_traversals = [self.wrapped_travesral_with_player(p) for p in range(self._num_players)]
    advantage_losses = collections.defaultdict(list)
    for _ in tqdm(range(self._num_iterations)):
      for p in range(self._num_players):
        with ThreadPoolExecutor() as executor:
          futures = [executor.submit(player_traversals[p]) for _ in range(self._num_traversals)]
          for future in futures:
              self.add(future.result())
