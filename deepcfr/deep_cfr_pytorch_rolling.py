...
class VisitBuffer():
  ...
  def _get_key(self, data):
    key = hashlib.md5(np.array(data).tobytes()).hexdigest()
    return key
  ...
  def get_keys_and_visits(self):
    keys = []
    visits = []
    for key in self._keys:
      keys.append(self._keys[key])
      visits.append(self._data[key])
    return keys, visits
...
class DeepCFRSolver(policy.Policy):
  ...
  def __init__(self,
    ...
    self._visit_buffer = VisitBuffer()
    self._policy_visit_counts = 0
    self._last_policy_visit_counts = 0
    ...
    self._optimizer_policy = torch.optim.Adam(
        self._policy_network.parameters(), lr=learning_rate*10)
    
    self._visit_count_network = MLP(self._embedding_size,
                               list(policy_network_layers),
                               1)
    self._visit_count_sigmoid = nn.Sigmoid()
    self._loss_visit_count = nn.MSELoss()
    self._optimizer_visit_count = torch.optim.Adam(
        self._visit_count_network.parameters(), lr=learning_rate*10)
  ...
  def _traverse_game_tree(self, state, player):
    ...
    if state.is_terminal():
      ...
      info_state_tensor = state.information_state_tensor(other_player)
      self._strategy_memories.add(
          StrategyMemory(
              info_state_tensor, self._iteration,
              strategy))
      self._visit_buffer.add(info_state_tensor)
      self._policy_visit_counts += 1
  ...
  def _learn_strategy_network(self):
    ...
    for s in samples:
      ...
      iterations.append([s.iteration])
    visit_info_states, num_visits = self._visit_buffer.get_keys_and_visits()
    self._optimizer_policy.zero_grad()
    
    ac_probs = torch.FloatTensor(np.array(np.squeeze(action_probs)))
    tensor_info_states = torch.FloatTensor(np.array(info_states))
    
    # Get current policy network output (this is the current cumulative average)
    logits = self._policy_network(tensor_info_states)
    current_average = self._policy_sm(logits)
    
    # Get visit counts for the states we're training on
    visit_counts_full = self._visit_count_sigmoid(self._visit_count_network(tensor_info_states))
    
    # Compute the target: rolling average update
    # new_average = old_average + (1/n) * (new_sample - old_average)
    # where n is proportional to visit_counts_full
    learning_rate_per_state = 1.0 / (visit_counts_full * self._last_policy_visit_counts + 1)
    target_average = current_average + learning_rate_per_state * (ac_probs - current_average)
    
    # The key fix: train the network to predict the TARGET, not itself
    loss_strategy = self._loss_policy(current_average, target_average.detach())
    
    loss_strategy.backward()
    self._optimizer_policy.step()
    
    # Now handle visit count network
    if len(visit_info_states) > 0:
        # Fix the optimizer reference
        self._optimizer_visit_count.zero_grad()
        
        visit_tensor = torch.FloatTensor(np.array(visit_info_states))
        num_visits_tensor = torch.FloatTensor(np.array(num_visits)).unsqueeze(1)
        
        # Get current predicted visit counts (old values)
        current_visit_counts = self._visit_count_sigmoid(self._visit_count_network(visit_tensor))
        
        # Convert to actual counts by scaling with total visits
        old_visit_counts = current_visit_counts * self._last_policy_visit_counts
        
        # Add new visits to old counts
        updated_visit_counts = old_visit_counts + num_visits_tensor
        
        # Normalize by new total visit count
        target_visit_counts = updated_visit_counts / max(1, self._policy_visit_counts)
        
        loss_visits = self._loss_visit_count(current_visit_counts, target_visit_counts.detach())
        loss_visits.backward()
        self._optimizer_visit_count.step()

    self._visit_buffer.clear()
    self._last_policy_visit_counts = self._policy_visit_counts
