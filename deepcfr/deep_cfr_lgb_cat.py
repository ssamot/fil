...
def get_convert_to_cat(cat_dims):

  def convert_to_cat(one_hot):
...
class DeepCFRSolver(policy.Policy):
  ...
  def __init__(self,
    ...
    self._convert_to_cat = get_convert_to_cat(self._cat_dims)
    ...
    X_dummy = np.zeros((2, len(self._cat_dims)))
  ...
  def _traverse_game_tree(self, state, player):
    ...
    if state.is_terminal():
      ...
      self._advantage_memories[player].add(
          AdvantageMemory(self._convert_to_cat(state.information_state_tensor()), self._iteration,
      ...
      self._strategy_memories.add(
          StrategyMemory(
              self._convert_to_cat(state.information_state_tensor(other_player)), self._iteration,
              strategy))
  ...
  def _sample_action_from_advantage(self, state, player):
    ...
    """
    info_state = self._convert_to_cat(state.information_state_tensor(player))
  ...
  def action_probabilities(self, state, player_id=None):
    ...
    info_state_vector = self._convert_to_cat(np.array(state.information_state_tensor()))
