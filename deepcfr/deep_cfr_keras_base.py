...
class DeepCFRSolver(policy.Policy):
  ...
  def _serialize_strategy_memory(self, info_state, iteration,
    ...
    return (info_state, iteration, strategy_action_probs, legal_actions_mask)
  ...
  def _serialize_advantage_memory(self, info_state, iteration, samp_regret,
    ...
    return (info_state, iteration, samp_regret, legal_actions_mask)
  ...
  def _traverse_game_tree(self, state, player):
    ...
    if state.is_terminal():
      ...
      self._add_to_strategy_memory(
          state.information_state_tensor(other_player), strategy, self._iteration,
  ...
  def _learn_advantage_network(self, player):
    ...
    for _ in range(self._advantage_network_train_steps):
      ...
      main_loss = self._advantage_train_step[player](*transformed_data, self._iteration)
  ...
  def _learn_strategy_network(self):
    ...
    for _ in tqdm(range(self._policy_network_train_steps)):
      ...
      entry_size = len(data[0])
      transformed_data = tuple([keras.ops.convert_to_tensor([data[i][j] for i in range(len(data))]) for j in range(entry_size)])
      main_loss = train_step(*transformed_data)
