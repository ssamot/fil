...
import pyspiel
from deepcfr import deep_cfr_public_state_regressors as deep_cfr
...
def main(unused_argv):
  ...
  for i in range(deep_cfr_solver._num_iterations):
    for p in range(deep_cfr_solver._num_players):
      ...
      deep_cfr_solver._learn_advantage_regressors(p)

    # Train policy network.
    deep_cfr_solver._learn_strategy_regressors()
  ...
  if FLAGS.index is not None:
    results_file = config.results_file_base + f"pt_splines_{FLAGS.index}.json"
  else:
    results_file = config.results_file_base + "pt_splines.json"
