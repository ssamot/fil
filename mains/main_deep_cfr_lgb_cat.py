...
import pyspiel
from deepcfr import deep_cfr_lgb_cat as deep_cfr
...
def main(unused_argv):
  ...
  for i in range(deep_cfr_solver._num_iterations):
    for p in range(deep_cfr_solver._num_players):
      ...
      advantage_losses[p].append(deep_cfr_solver._learn_advantage_model(p))

    # Train policy network.
    policy_loss = deep_cfr_solver._learn_strategy_model()
  ...
  results_file = config.results_file_base + "lgb_cat.json"
